...
.PHONY: all test clean deploy fund help install snapshot format anvil deploy-fund-approve approve-routers-for-swapper create-muddy-pool create-clear-pool add-liquidity remove-liquidity swap collect-fees whitelist-router remove-whitelist-router add-policy add-IdDocs-to-token add-token remove-token test-sepolia-fork-clean test-sepolia-fork-current test-anvil test-all test-clean test-current
