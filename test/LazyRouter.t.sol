...
contract LazyRouterTest is Test, Deployers, KYCEvents {
    ...
    function setUp() public {
        ...
        routersToWhitelist[0] = address(s_contracts.lazyRouter);
        s_contracts.kycHook1.updateRouterWhitelist(routersToWhitelist, true);

        // Deploy LazyRouter
        vm.prank(KYC_ROUTER_OWNER);
        s_contracts.lazyRouter = new LazyRouter(s_contracts.poolManager);
        console.log("LazyRouter contract deployed with address: %s", address(s_contracts.lazyRouter));
        ...
        address[3] memory toApprove = [
            ...
            address(s_contracts.lazyRouter)
        ];
        ...
        routersToWhitelist2[0] = address(s_contracts.lazyRouter);
        ...
        // Approve PoolManager on behalf of LazyRouter for both tokens
        ...
        console.log("Approved PoolManager on behalf of LazyRouter for both tokens");
        vm.stopPrank();
    ...
    function test_setup() public {
        ...
        assertEq(
            MockERC20(Currency.unwrap(currency0)).allowance(s_swapper, address(s_contracts.lazyRouter)),
            type(uint256).max
        );
        assertEq(
            MockERC20(Currency.unwrap(currency1)).allowance(s_swapper, address(s_contracts.lazyRouter)),
            type(uint256).max
        );
        assertEq(
        ...
        assert(s_contracts.kycHook1.isRouterWhitelisted(address(s_contracts.lazyRouter)));
    ...
    function test_swapFailsWithBadPolicy() public {
        ...
        vm.prank(s_swapper);
        vm.expectRevert();
        LazyRouter.SwapSettings memory settings = LazyRouter.SwapSettings({takeClaims: false, settleUsingBurn: false});
        IPoolManager.SwapParams memory swapParams = IPoolManager.SwapParams({
            zeroForOne: true,
            amountSpecified: 0.0001 ether,
            sqrtPriceLimitX96: TickMath.MIN_SQRT_PRICE + 1
        });
        s_contracts.lazyRouter.swap(s_poolKey1, swapParams, settings, "");
    }

    function test_swap_lazyRouter() public {
        // Add liquidity to the pools
        ...
        emit LazySwapAttemptThroughRouter(s_poolId2, address(s_contracts.lazyRouter), s_swapper, s_poolKey2, swapParams);
        s_contracts.lazyRouter.swap(s_poolKey2, swapParams, settings, "");
        ...
        console.log("Swapped through non-KYC Pool with LazyRouter");
        ...
        vm.prank(s_swapper);
        vm.expectEmit(true, true, true, true);
        emit LazySwapAttemptThroughRouter(s_poolId1, address(s_contracts.lazyRouter), s_swapper, s_poolKey1, swapParams);
        s_contracts.lazyRouter.swap(s_poolKey1, swapParams, settings, hookDataFromSwapperAddress);
        ...
        assertGt(beforeBalanceToken0, afterBalanceToken0);
        assertLt(beforeBalanceToken1, afterBalanceToken1);
        console.log("Swapped through KYC Pool with LazyRouter");
    }

    function test_swap_lazyRouter_withBlacklistedUser() public {
        setupPoolsWithLiquidity();
        ...
        console.log("Swap through KYC Pool with LazyRouter failed for blacklisted user");
    }

    function test_swap_lazyRouter_withMalformedHookData() public {
        setupPoolsWithLiquidity();
        ...
        bytes memory malformedHookData = abi.encode(uint256(123)); // Incorrect format

        vm.prank(s_swapper);
        BalanceDelta delta = s_contracts.lazyRouter.swap(s_poolKey1, swapParams, settings, malformedHookData);

        // The swap should still succeed, as the LazyRouter falls back to the original hookData
        assert(delta.amount0() != 0 || delta.amount1() != 0);
        console.log("Swap through KYC Pool with LazyRouter succeeded with malformed hookData");
    }

    function test_LazySwapAttemptThroughRouterEvent() public {
        setupPoolsWithLiquidity();
        ...
        console.log("LazySwapAttemptThroughRouter event emitted correctly");
    }
